...
public class LzoDecompressor implements Decompressor {
  ...
  private long lzoDecompressor = 0;   // not the actual lzo decompression function.
  ...
  throws IOException {
   System.out.println("decompress function triggered in lzodecompressor class...\n"); 
   if (b == null) {
      throw new NullPointerException();
    }
    if (off < 0 || len < 0 || off > b.length - len) {
    ...
    if (isCurrentBlockUncompressed()) {
      ...
      // copy directly from the input buffer(userBuf).
